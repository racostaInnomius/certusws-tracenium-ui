...
export function KeyVaultConnectorForm({ onCreated, disabled }) {
  const [label, setLabel] = React.useState("");
  const [vaultUrl, setVaultUrl] = React.useState("");
  ...
  const ready = label.trim().length >= 2 && /^https:\/\//i.test(vaultUrl.trim()) && tenantId.trim() && clientId.trim() && clientSecret;

  const submit = async () => {
    setBusy(true);
    setError(null);
    try {
      const r = await createCdpConnector({
        kind: "keyvault",
        label: label.trim(),
        config: { vaultUrl: vaultUrl.trim(), tenantId: tenantId.trim(), clientId: clientId.trim() },
        clientSecret
      });
  ...
  return (
    <Box>
      <Stack direction="row" spacing={1} sx={{ flexWrap: "wrap", rowGap: 1 }}>
        <TextField size="small" label="Label" value={label} onChange={(e) => setLabel(e.target.value)} placeholder="Production vault" sx={{ minWidth: 160 }} disabled={disabled} />
        <TextField size="small" label="Vault URL" value={vaultUrl} onChange={(e) => setVaultUrl(e.target.value)} placeholder="https://kv-prod.vault.azure.net" sx={{ minWidth: 280 }} disabled={disabled} />
        <TextField size="small" label="Directory (tenant) ID" value={tenantId} onChange={(e) => setTenantId(e.target.value)} sx={{ minWidth: 300 }} disabled={disabled} />
        <TextField size="small" label="Application (client) ID" value={clientId} onChange={(e) => setClientId(e.target.value)} sx={{ minWidth: 300 }} disabled={disabled} />
        <TextField size="small" label="Client secret" type="password" value={clientSecret} onChange={(e) => setClientSecret(e.target.value)} sx={{ minWidth: 220 }} disabled={disabled} autoComplete="off" />
        <Button size="small" variant="contained" disabled={disabled || busy || !ready} onClick={submit}>
          {busy ? "Saving…" : "Add Key Vault"}
        </Button>
      </Stack>
  ...
  return (
    <Box sx={{ mt: 2, pt: 1.5, borderTop: `1px dashed ${BRAND.border}` }}>
      ...
      <Typography sx={{ fontSize: TEXT.sm, color: BRAND.dark, opacity: 0.8, mb: 1 }}>
        ...
        keys (type, size, HSM, expiry); a vault certificate that also lives on a device is matched by fingerprint.
      </Typography>
      ...
      <KeyVaultConnectorForm disabled={!secretsConfigured} onCreated={() => { reload(); }} />
          ...
          {connectors.map((c) => (
            <Box key={c.connectorId} sx={{ border: `1px solid ${BRAND.border}`, borderRadius: 1.5, p: 1.25 }}>
              <Stack direction="row" spacing={1} alignItems="center" sx={{ flexWrap: "wrap", rowGap: 0.5 }}>
                ...
                <Typography sx={{ fontSize: TEXT.xs, color: BRAND.gray }}>{c.config?.vaultUrl}</Typography>
                <StatusChip c={c} />
