...
describe("useBulkSelection — bulk runners", () => {
  ...
  it("all ok → success toast 'label: n/n ok', clears selection, refetches", async () => {
    ...
    act(() => result.current.handleBulkRequestException());
    expect(result.current.bulkExceptionOpen).toBe(true);
    await act(() => result.current.confirmBulkException({ kind: "risk_accepted", justification: "Legacy ERP until the Q4 migration", riskOwner: "admin@certusitm.com", expiresAt: "2027-01-01T00:00:00.000Z" }));
    expect(result.current.bulkExceptionOpen).toBe(false);
    expect(bulkFindingOp).toHaveBeenCalledWith({ op: "request_exception", ...{ kind: "risk_accepted", justification: "Legacy ERP until the Q4 migration", riskOwner: "admin@certusitm.com", expiresAt: "2027-01-01T00:00:00.000Z" }, findingIds: [1, 2] });
    expect(onToast).toHaveBeenCalledWith({ severity: "success", message: "Exception requested: 2/2 ok" });
    expect(result.current.selectedIds.size).toBe(0);
  ...
  it("bulk change-status goes through the dialog and sends newStatus + note", async () => {
    ...
    act(() => result.current.handleBulkChangeStatus("in_progress"));
    expect(result.current.bulkStatusDialog).toEqual({ targetStatus: "in_progress" });
    await act(() => result.current.confirmBulkStatusChange({ note: "scheduled for Friday" }));
    expect(bulkFindingOp).toHaveBeenCalledWith({ op: "change_status", newStatus: "in_progress", note: "scheduled for Friday", findingIds: [3] });
