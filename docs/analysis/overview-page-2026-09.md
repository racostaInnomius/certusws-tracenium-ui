...
| O9 | Filtros de Assets en servidor + navegación sin arrastrar filtros | backend + UI | hecho (`d71f272` + `0e19984` + `be12678`) | Cierra los dos pendientes de O8. **Assets**: plataforma, versión y grupo filtraban la página cargada (25 de 53; Older 4 → 2). `/dashboard/hosts` filtra en SQL antes del COUNT/LIMIT (`hosts-list-filters.itest.ts`, 10 casos); la UI traduce el grupo de versiones a versiones exactas con `versionsInBucket` sobre los datos de la dona, y la regla vive sólo en `bucketOfAgentVersion`. Respaldo en navegador si el backend no devuelve `filters`. **Navegación**: seis sitios armaban la URL de otra página copiando la actual (menú, campana, Overview, SCP, PMP, Welcome); pasan por `searchForPage`, que conserva sólo `page`, los parámetros del enlace y `*AutoRefresh`. Test de guardia contra el séptimo. |
| O10 | Validación en el portal tras desplegar | backend + UI | hecho (`d35f4a1` + `dacbc65` + `f670313` + `4212f65`) | T111, 14-sep 00:12 UTC. ✅ Reports → History (8 of 8); Fleet composition → Hardware Inventory, Laptops 26 = 1–10 of 26; Assets versión 4/3/46 = dona; grupo estático de 30 → 30 total; grupo dinámico 11 → 11; gráfica de Jobs → since=7d. ❌→arreglado: **Failed jobs 4, lista 5** — la serie cuenta días naturales UTC y `since` era N×24 h (el 5.º, 7-sep 01:36 UTC); `since` usa ya la expresión de la serie, itest compara lista contra la serie. **Audit 959 vs 963** — el enlace mandaba la medianoche LOCAL; ahora el inicio del día UTC (con `auditFrom` UTC da 959 exacto). **Chip de grupo (25) con 30 total** — contaba la primera página de miembros. Segmentos de Agent versions vuelven a filtrar. ⚠️ `be12678` (navegación limpia) NO estaba en el despliegue: el bundle aún copia la URL en Topbar y SCP; pendiente de validar. |
