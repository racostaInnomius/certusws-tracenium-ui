...
| U6 | Catálogo agrupado por PÁGINA, no por informe | UI | hecho (`72e8e25`) | Seis informes para ONCE páginas con botón "Report": la lista por informe escondía las cinco ausencias. Una fila por página en el orden del menú, con la cuenta y los informes plegados dentro. Medido: 11 filas en 645 px sin scroll, 6 huecos visibles. ⚠️ Una fila vacía dice qué abre hoy su botón; y `isEntitled` separa "no construido" de "no contratado", que en una fila que dice 0 se leen igual y son opuestos. |
| U7 | El botón "Report" aterriza en SU página | UI | hecho (`d32e12f`) | `?reportKey=` despliega la fila de la página de origen, con sus 1–2 informes a la vista, en vez de confirmar y generar solo. Aquel camino elegía el formato sin preguntar y escondía el segundo informe de la página (Evidence Pack). ⚠️ Bug previo: la URL se limpiaba ANTES de leer `reportParams`/`reportFormat`, así que el framework de Security Compliance nunca llegó al informe; ahora se lee primero y un test lo fija. |
